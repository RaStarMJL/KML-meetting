export default {
  ...
  "Please accept the privacy policy and user agreement!":
    "请同意隐私协议及用户协议！",
  "Incorrect verification code, please check the code!":
    "验证码错误，请检查验证码！",
  "The verification code has expired, please retrieve a new one!":
    "验证码已过期，请重新获取验证码！",
  "The verification code has been used, please retrieve a new one!":
    "验证码已使用，请重新获取验证码！",
  ...
  // @ts-ignore
  "sb has been set as administrator": ({ named }) =>
    `已将 ${named("name")} 设为管理员`,
  ...
  "You can turn on the microphone and camera once you are on stage":
    "上台后可以打开麦克风和摄像头",
  // @ts-ignore
  "Sb invites you to speak on stage": ({ named }) =>
    `${named("role")}邀请您上台发言`,
  ...
  "Step down": "下台",
  // @ts-ignore
  "sb has rejected your application for the stage": ({ named }) =>
  ...
  // @ts-ignore
  "Sb invites you to turn on the microphone": ({ named }) =>
    `${named("role")}邀请你打开麦克风`,
  ...
  // @ts-ignore
  "Reject on Stage failed, please retry": ({ named }) =>
    `拒绝 ${named("name")} 上台失败, 请重试`,
  // @ts-ignore
  "Agree on Stage failed, please retry": ({ named }) =>
  ...
  "The current browser does not support audio and video communication capabilities":
    "当前浏览器不支持音视频通信能力",
  // @ts-ignore
  "whether to kick sb off the room": ({ named }) =>
    `是否将 ${named("name")} 移出房间`,
  // @ts-ignore
  "An invitation to open the microphone has been sent to sb.": ({ named }) =>
  ...
  // @ts-ignore
  "The room owner has been transferred to sb": ({ named }) =>
    `已将房主转交给${named("name")}`,
  ...
  "X people are in the meeting": ({ values }: any) =>
    `${values?.number} 人在会议中`,
  "Already joined": "已入会",
  ...
  Attention: "注意",
  'Media playback failed. Click the "Confirm" to resume playback':
    "音视频播放失败，请点击“确认”按钮恢复播放。",
};
